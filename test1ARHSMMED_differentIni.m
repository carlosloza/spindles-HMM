...
dState = round(normrnd(muk(zaux), sigk(zaux)));
daux{zaux} = [daux{zaux} dState];
dStateall = dState;
...
edges = 0.5:1:max(dStateall)+0.5;
dcount = zeros(K, max(dStateall));
for k = 1:K
    [f, xi] = ksdensity(daux{k}, 1:max(dStateall));
    dcount(k, :) = f;
    
    %[N, ~] = histcounts(daux{k}, edges);
    %dcount(k, :) = N/sum(N);
end
...
DurationParameters.dmax = max(dStateall);

% Nonparametric
DurationParameters.PNonParametric(1,:) = dcount(2,:);
DurationParameters.PNonParametric(2,:) = dcount(1,:);
DurationParameters.PNonParametric(3,:) = dcount(3,:);

% Gaussian
%DurationParameters.mu = [80 50 100];
%DurationParameters.sigma = [5 2 8];
