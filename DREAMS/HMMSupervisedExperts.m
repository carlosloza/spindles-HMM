function HMMSupervisedExperts(ObsModel, p, robustMstepflag, dmaxSec)
% dmax in seconds
% preproc = 0 - no preprocessing
% preproc = 1 - preprocessing, dual basis pursuit
% dmax in seconds
...
% duration distribution for spindles) for each one of the two models
...
if robustMstepflag == true
    save(['ICASSP results/Experts/' ObsModel '/AR order ' num2str(p)...
        '/Robust_dmax' num2str(dmaxSec) 'sec.mat'], 'PerfTestExpert', 'NLLTest', 'HMModelSupervised')
else
    save(['ICASSP results/Supervised/EM/' ObsModel '/AR order ' num2str(p)...
        '/NoRobust_dmax' num2str(dmaxSec) 'sec.mat'], 'PerfTestExpert', 'NLLTest', 'HMModelSupervised')
end
