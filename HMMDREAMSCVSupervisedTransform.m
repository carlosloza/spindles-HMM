...
% Data is pre-processed by the dual Basis Pursuit Denoising
...
HMModelIni.ObsParameters.meanModel = 'MLP';
    ...
    for p_i = 1:length(p_v)
        ...
        for i = 1:numel(trainSet)
            ...
            for k = 1:numel(truetrainSet)
                load(['DREAMS/Subject' num2str(truetrainSet(k)) '_Fs' num2str(Fs) '_Transformed.mat'])
            ...
            for k = 1:numel(valSet)
                load(['DREAMS/Subject' num2str(valSet(k)) '_Fs' num2str(Fs) '_Transformed.mat'])
                ytest = y_osc;
                %ytest = [0 diff(y)];
...
if strcmpi(HMModel.type, 'HMM') || strcmpi(HMModel.type, 'HSMMED')
    ...
    save(['DREAMS/Results/Transform Univariate/' HMModel.type '_Supervised' rstring...
        '_Mean_' HMModel.ObsParameters.meanModel HMModel.ObsParameters.model '.mat'],...
